...
#' @export
grouplasso2pop_linreg_obj <- function(beta1,beta2,Y1,X1,groups1,Y2,X2,groups2,lambda,eta,w1,w2,w,AA1,AA2,Com)
{
  ...
  SSE1 <- (1/2) * sum(  ( Y1 - X1 %*% beta1 )^2 )
  SSE2 <- (1/2) * sum(  ( Y2 - X2 %*% beta2 )^2 )
  ...
  pen1 <- lambda * (beta1.wl2l1 + (n2/n1) * beta2.wl2l1 )
...
grouplasso2pop_linreg <- function(Y1,X1,groups1,Y2,X2,groups2,lambda,eta,w1,w2,w,AA1,AA2,Com,tol=1e-4,max.iter=500,plot_obj=FALSE,init=NULL)
{
  ...
  got.eigen2 <- numeric(q2)
  eigen1 <- vector("list", q1)
  eigen2 <- vector("list", q2)
  ...
  while( conv > tol & iter < max.iter)
  {
    ...
    for(j in nonsingleton.grps1)
    {
      ...
      if(j %in% Com)
      {
        ...
        if(x1r1j.AAb2.norm < lambda * w1[j])
        {
          ...
          if(got.eigen1[j]==0)
          {
            ...
            LtL <- t(X1[,ind1]) %*% X1[,ind1] + eta * w[j] * t(AA1[[j]])%*% AA1[[j]]
            eigen1[[j]] <- eigen( LtL )
            got.eigen1[j] <- 1
    ...
    # rescale the lambda for the second dataset
    lambda2 <- n2/n1*lambda
      ...
      if(j %in% Com)
      {
        ...
        if(x2r2j.AAb1.norm < lambda2 * w2[j])
        {
          ...
          if(got.eigen2[j]==0)
          {
            ...
            LtL <- t(X2[,ind2]) %*% X2[,ind2] + eta * w[j] * t(AA2[[j]])%*% AA2[[j]]
            eigen2[[j]] <- eigen( LtL )
            got.eigen2[j] <- 1
    ...
    if(plot_obj == TRUE)
    {
      obj.val[iter] <- grouplasso2pop_linreg_obj(beta1.hat1,beta2.hat1,Y1,X1,groups1,Y2,X2,groups2,lambda,eta,w1,w2,w,AA1,AA2,Com)
    }
...
#' Fit grouplasso2pop regression estimator with continuous responses over a grid of lambda and eta values
#'
...
grouplasso2pop_linreg_grid <- function(Y1,X1,groups1,Y2,X2,groups2,n.lambda,n.eta,lambda.min.ratio,w1,w2,w,AA1,AA2,Com,tol=1e-4,max.iter=500,report.prog=FALSE,grid.only=FALSE)
{
  ...
  for(j in 2:max(q1,q2))
  {
    ...
    if(j <= q1)
    {
      norms1[j] <- sqrt(sum((t(X1[,ind1]) %*% (Y1 - mean(Y1)))^2)) / w1[j]
    }
    if(j <= q2)
    {
      norms2[j] <- sqrt(sum((t(X2[,ind2]) %*% (Y2 - mean(Y2)))^2)) / w2[j] * n1 / n2
    }
  ...
  step <- 0
  init <- NULL
  for(l in 1:ifelse(grid.only,1,n.lambda)){
    for(k in 1:n.eta){
      ...
      if(l == 1 & k == 1){# define the eta sequence
        ...
        SSE1 <- (1/2) * sum(  ( Y1 - X1 %*% b1 )^2 )
        SSE2 <- (1/2) * sum(  ( Y2 - X2 %*% b2 )^2 )
  ...
  if(grid.only == TRUE)
  {
    output <- list( lambda.seq = lambda.seq,
                    eta.seq = eta.seq)
...
#' Fit grouplasso2pop continuous-response regression estimator over a user-specified grid of lambda and eta values
#'
#' @param Y1 the continuous response vector of data set 1
#' @param X1 matrix containing the design matrices for data set 1
#' @param groups1 a vector indicating to which group each covariate of data set 1 belongs
  ...
  for(l in 1:n.lambda){
    for(k in 1:n.eta){
      ...
      for(fold in 1:n.folds){
        ...
        grouplasso2pop_linreg.out <- grouplasso2pop_linreg(Y1[-fold1],
                                                           X1[-fold1,],
                                                           groups1,
                                                           Y2[-fold2],
                                                           X2[-fold2,],
        ...
        P1.fold <- X1[fold1,] %*% b1.fold
        P2.fold <- X2[fold2,] %*% b2.fold

        SSE1.fold <- sum( (Y1[fold1] - P1.fold )^2 )
        SSE2.fold <- sum( (Y2[fold2] - P2.fold )^2 )

        minus2ll.arr[l,k,fold] <- SSE1.fold + SSE2.fold
...
#' @param X1 matrix containing the design matrices for data set 1
#' @param groups1 a vector indicating to which group each covariate of data set 1 belongs
#' @param Y2 the continous response vector of data set 2
#' @param X2 matrix containing the design matrices for data set 2
#' @param groups2 a vector indicating to which group each covariate of data set 2 belongs
#' @param n.lambda the number of lambda values desired
#' @param n.eta the number of eta values desired
...
#' n.folds <- 6
#'
#' grouplasso2pop_linreg_grid_cv.out <- grouplasso2pop_linreg_cv(Y1,X1,groups1,Y2,X2,groups2,n.lambda,n.eta,lambda.min.ratio,n.folds,w1,w2,w,AA1,AA2,Com)
#'
#' grouplasso2pop_linreg_grid_cv.out$lambda.seq
...
#' n.folds <- 6
#'
#' grouplasso2pop_linreg_grid_cv_adapt.out <- grouplasso2pop_linreg_cv_adapt(Y1,X1,groups1,Y2,X2,groups2,n.lambda,n.eta,lambda.min.ratio,n.folds,w1,w2,w,AA1,AA2,Com)
#'
#' grouplasso2pop_linreg_grid_cv_adapt.out$lambda.seq
...
grouplasso2pop_linreg_cv_adapt <- function(Y1,X1,groups1,Y2,X2,groups2,n.lambda,n.eta,lambda.min.ratio,n.folds,w1,w2,w,AA1,AA2,Com,tol=1e-3,max.iter=500,report.prog = TRUE){
  ...
  # obtain lambda.seq and eta.seq from the grid function, as well as the fits on the entire data set, which will be used as initial values for the crossvalidation training fits.
  grouplasso2pop_linreg_grid.out <- grouplasso2pop_linreg_grid(Y1,
                                                               X1,
                                                               groups1,
                                                               Y2,
                                                               X2,
  ...
  # do the crossvalidation
  grouplasso2pop_linreg_cv_fixedgrid.out <- grouplasso2pop_linreg_cv_fixedgrid(Y1,
                                                                               X1,
                                                                               groups1,
                                                                               Y2,
                                                                               X2,
...
grouplasso2pop_linreg_cv_adapt_fixedgrid <- function(Y1,X1,groups1,Y2,X2,groups2,lambda.seq,eta.seq,lambda.initial.fit,n.folds,w1,w2,w,AA1,AA2,Com,tol=1e-3,max.iter=500)
{
  ...
  # obtain lambda.seq and eta.seq from the grid function, as well as the fits on the entire data set, which will be used as initial values for the crossvalidation training fits.
  grouplasso2pop_linreg_grid.out <- grouplasso2pop_linreg_fixedgrid(Y1 = Y1,
                                                                    X1 = X1,
                                                                    groups1 = groups1,
                                                                    Y2 = Y2,
                                                                    X2 = X2,
